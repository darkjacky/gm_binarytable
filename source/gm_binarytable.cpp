				...
				case (GarrysMod::Lua::Type::NUMBER): {
					...
					if ((in - (long long)in) == 0) {											// Check if number is an integer
						...
						case (sizeof(signed char)): {											// Can be stored as char
							...
							stream.write(TYPECHAR, (signed char)out);							// Write number to stream
							break;
						}
						case (sizeof(short)): {													// Can be stored as short
							short out = in;														// Turn our variable into a short
							stream.write(TYPESHORT, (short)out);								// Write number to stream
							break;
						}
						case (sizeof(int)): {													// Can be stored as int
							int out = in;														// Turn our variable into a int
							stream.write(TYPEINT, (int)out);									// Write number to stream
							break;
						}
						default:
							stream.write(TYPENUMBER, in);										// Write number to stream
...
public:
	...
	template<class T>
	T read() {
		unsigned int len = sizeof(T);															// Size variable
		position += len;																		// Increase position by len
		return *(T*)(str + (position - len));													// Big hacker time. return the reference of a pointer from the string + position - length (A lot quicker than creating a variable then memcpy)
	}
