...
class QuickStrWrite {
public: QuickStrWrite() : length( 1048576 ) {
...
private: unsigned int position = 0; // Position we are currently at
...
class QuickStrRead {
public: QuickStrRead( const char* in, unsigned int& len ) {
    ...
    length = len; // Copy length
      ...
      void read( T* to, unsigned int len ) {
          ...
          to = (T*) const_cast < char* > (str + position); // Stop you have violated the law
          position += len; // Offset position
      }

      void ReadString( unsigned int& len,
          const char* outstr, unsigned int readsize ) {
          read( &len, readsize ); // Read length
          outstr = str + position; // Edit our const char pointer to be the exact pointer we are currently using
          position += len; // Offset position
      }
...
private: unsigned int position = 0; // Position we are reading the string from
                ...
                case (TYPESTRINGCHAR):
                    {
                        ...
                        const char* str; // Create pointer
                        stream.ReadString( len, str, sizeof( unsigned char ) ); // Push len variable, pointer, header length
                ...
                case (TYPESTRINGSHORT):
                    {
                        unsigned int len = 0; // Create length variable
                        const char* str; // Create pointer
                        stream.ReadString( len, str, sizeof( unsigned short ) ); // Push len variable, pointer, header length
                        LUA->PushString( str, len ); // Push string to Lua with length
                        break;
                    }
                case (TYPESTRINGLONG):
                    {
                        unsigned int len = 0; // Create length variable
                        const char* str; // Create pointer
                        stream.ReadString( len, str, sizeof( unsigned int ) ); // Push len variable, pointer, header length
                        LUA->PushString( str, len ); // Push string to Lua with length
                        break;
                    }
                case (TYPEVECTOR):
